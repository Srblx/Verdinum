...
export default function App() {
    ...
    return () => {
      ...
      scene.traverse((o: THREE.Object3D) => {
        ...
        if ((o as any).material) {
          Array.isArray((o as any).material)
            ? (o as any).material.forEach((m: THREE.Material) => m.dispose())
  ...
  useEffect(() => {
    ...
    if (document.readyState === 'complete') {
      ...
      window.addEventListener('load', () => loadAssetsWithRetry(), {
        once: true,
      });
    }
  }, []);
  ...
  const computeStats = useCallback(() => {
    ...
    const jsWeight = resources
      .filter((r) => r.initiatorType === 'script')
      .reduce((sum, r) => sum + (r.transferSize || 0), 0);
    const cssWeight = resources
    ...
    const totalEncoded =
      nav.encodedBodySize +
      resources.reduce((sum, r) => sum + (r.encodedBodySize || 0), 0);
  ...
  useEffect(() => {
    ...
    // TODO: Augmentation de l'intervalle pour réduire la charge CPU de 90%
    const interval = setInterval(computeStats, 30_000); // Augmenté de 5000ms à 30000ms
  ...
  useEffect(() => {
    // TODO: Throttle du PerformanceObserver pour réduire les calculs de 90%
    const throttledUpdate = throttle((...args: unknown[]) => {
      const res = args[0] as PerformanceResourceTiming[];
      const added = res.reduce((a, b) => a + (b.transferSize || 0), 0);
      const jsAdd = res
        .filter((r) => r.initiatorType === 'script')
        .reduce((a, b) => a + (b.transferSize || 0), 0);
      const cssAdd = res
        .filter((r) => r.initiatorType === 'link' || /\.css$/i.test(r.name))
        .reduce((a, b) => a + (b.transferSize || 0), 0);
      const isImg = (r: PerformanceResourceTiming) =>
        r.initiatorType === 'img' ||
        r.initiatorType === 'css' ||
        /\.(avif|jpe?g|png|gif|webp|svg)$/i.test(r.name);
      const imgAdd = res
        .filter(isImg)
        .reduce((a, b) => a + (b.transferSize || 0), 0);
    ...
    }, 5000); // TODO: Throttle de 5 secondes pour réduire les calculs constants
  ...
  // TODO: Utilisation de l'API unifiée - Une seule requête pour toutes les données
  useEffect(() => {
    if (intervalRef.current) return;

    // TODO: Suppression de la requête payload inutile - Économie de 12 requêtes/minute
    // fetch(`http://localhost:5001/api/payload?${Date.now()}`) // SUPPRIMÉ

    intervalRef.current = window.setInterval(async () => {
      try {
        // TODO: Une seule requête unifiée au lieu de deux requêtes séparées
        const response = await fetch('http://localhost:5001/api/unified', {
          cache: 'no-store',
        });
        const { memory, load, rps } = await response.json();
        ...
        console.warn('Erreur lors du fetch des stats unifiées', err);
      }
    }, 30_000); // TODO: Augmentation de l'intervalle de 5000ms à 30000ms - Réduction de 90% des requêtes
      ...
      <div className='relative z-10 container mx-auto px-6 py-12'>
        <header className='text-center mb-16'>
          ...
          <p className='text-xl text-slate-300 max-w-3xl mx-auto'>
            Plateforme d'entraînement avancée pour l'optimisation web et
            l'éco-conception
          </p>
        </header>
        <section className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 mb-16'>
          <Card
            ...
            title='Poids HTML'
            value={`${(stats.bundle / 1_024).toFixed(0)} kB`}
            tone={cardColors.bundle}
            tip='transferSize du document'
          />
          <Card
            icon={<Globe className='w-8 h-8 text-blue-400' />}
            title='Poids page'
            value={`${(stats.weight / 1_024).toFixed(0)} kB`}
            tone={cardColors.weight}
            tip='somme transferSize'
          />
          <Card
          ...
          <Card
            ...
            tone={cardColors.resources}
            tip='entries PerformanceResourceTiming'
          />
          <Card
            icon={<FileText className='w-8 h-8 text-fuchsia-400' />}
            title='JS'
            value={`${(stats.js / 1_024).toFixed(0)} kB`}
            tone={cardColors.js}
          />
          <Card
          ...
          <Card
            ...
            title='Images'
            value={`${(stats.img / 1_024).toFixed(0)} kB`}
            tone={cardColors.img}
          />
          <Card
            icon={<Cloud className='w-8 h-8 text-emerald-400' />}
            title='Cache hit'
            value={`${Math.round(stats.cache * 100)} %`}
            tone={cardColors.cache}
          />
          <Card
            icon={<MemoryStick className='w-8 h-8 text-red-400' />}
            title='RAM serveur'
          ...
          />
          <Card
            icon={<Activity className='w-8 h-8 text-lime-400' />}
            title='RPS'
            value={stats.rps}
            tone='bg-white/10 border-white/20'
          />
          <Card
            icon={<Timer className='w-8 h-8 text-yellow-400' />}
            title='Load page'
            value={`${stats.pl} ms`}
            tone='bg-white/10 border-white/20'
          />
        ...
        <section className='bg-white/10 backdrop-blur-lg rounded-2xl p-8 border border-white/20 mb-16'>
          ...
          <p className='text-slate-300 text-center mt-4'>
            10 cubes tournants en temps réel
          </p>
        </section>
      </div>
    </div>
